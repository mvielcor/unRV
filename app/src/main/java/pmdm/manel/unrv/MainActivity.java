...
public class MainActivity extends AppCompatActivity {
    ...
    ElMeuPrimerAdaptador empa;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        //Inicialitzem l'arraylist amb les dades dels cicles de florida
        llistat_titulacions =  new ArrayList<CicleFlorida>();
        creaDades();

        rv=(RecyclerView) findViewById(R.id.recyclerCicles);
        llm = new GridLayoutManager(getApplicationContext(),2);
        rv.setLayoutManager(llm);
        empa = new ElMeuPrimerAdaptador(llistat_titulacions);
        rv.setAdapter(empa);
